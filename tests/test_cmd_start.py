...
class TestCmdStart:
    ...
    async def _test_cmd_start(
        ...
        bot.add_result_for(method=SendMessage, ok=True, result=Message(message_id=1, date=datetime.now(), chat=chat))
        ...
        result = await dp.feed_update(bot, Update(message=message, update_id=1), _translator_hub=translator_hub)
    ...
    async def test_cmd_start_en(
        ...
        await self._test_cmd_start(translator_hub=translator_hub, dp=dispatcher, bot=bot_mock)
    ...
    async def test_cmd_start_de(
        ...
        await self._test_cmd_start(translator_hub=translator_hub, dp=dispatcher, bot=bot_mock, language_code="de")
